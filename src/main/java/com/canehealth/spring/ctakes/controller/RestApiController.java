...
public class RestApiController {
    ...
    public ResponseEntity<?> post_text_body(@RequestBody String text) throws Exception {
        logger.info(text);
        return new ResponseEntity<>(ctakesService.Jcas2json(text), HttpStatus.OK);
    }

    @RequestMapping(value = "/api", method = RequestMethod.GET)
    public ResponseEntity<?> get_text(@RequestParam(value = "text", required = false) String text) throws Exception {
        logger.info(text);
        return new ResponseEntity<>(ctakesService.Jcas2json(text), HttpStatus.OK);
    ...
    public ResponseEntity<?> post_text(@RequestParam("text") String text) throws Exception {
        logger.info(text);
        return new ResponseEntity<>(ctakesService.Jcas2json(text), HttpStatus.OK);
